...
from .constants import XARM_CONFIG_FILE
...
class Device:
    def __init__(self, serial_number=None):
        ...
        if platform.system() == 'Linux':
            ...
            devices = en.find(vid=0x0483, pid=0x5750, serial=serial_number)
            if len(devices) == 0:
            ...
            try:
                self.device.open(0x0483, 0x5750)
            except:
                print('[ERROR] No device found. Ensure that the XARM is connected via '
                      'usb cable and the power is on.\n'
                      'Turn the XARM off and back on again if needed.')
            ...
            import hid
            self.device = hid.device()
            self.device.open(0x0483, 0x5750, serial_number)
